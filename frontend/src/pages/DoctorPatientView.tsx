...
const SectionHeader: React.FC<{ icon: React.ReactNode; title: string; open: boolean; onClick: () => void }> = ({ icon, title, open, onClick }) => (
    <button
        onClick={onClick}
...
const DoctorPatientView: React.FC = () => {
    ...
    const [openSections, setOpenSections] = useState<Record<Section, boolean>>({
        profile: true, history: true, metrics: false, appointments: false, ip: true,
    });
    ...
    const fetchPatientData = async (id: string) => {
        ...
        try {
            const [patRes, recordsRes, metricsRes, aptsRes] = await Promise.allSettled([
                patientsAPI.getById(id),
                patientsAPI.getMedicalRecords(id),
            ...
            else { setError('Patient not found. Please check the Patient ID.'); setIsLoading(false); return; }
            ...
            // Fetch IP management records from a dedicated collection
            try {
                const ipRes = await api.get(`/patients/${id}/ip-records`);
    ...
    const handleAddIPRecord = async (e: React.FormEvent) => {
        ...
        setIpSuccess('');
        try {
            const newRecord: IPEntry = {
                admission_date: ipForm.admission_date || new Date().toISOString(),
                reason: ipForm.reason || '',
                ward: ipForm.ward,
                bed_number: ipForm.bed_number,
                attending_doctor: ipForm.attending_doctor,
                notes: ipForm.notes,
                status: ipForm.status as 'admitted' | 'discharged',
                discharge_date: ipForm.status === 'discharged' ? (ipForm.discharge_date || new Date().toISOString()) : undefined,
            };
            // Save to backend (or store locally if the backend doesn't have the route yet)
            try {
                await api.post(`/patients/${patient.id}/ip-records`, newRecord);
            } catch {
                // Fallback: store locally if endpoint doesn't exist
            }
            setIpHistory(prev => [{ ...newRecord, id: Date.now().toString() }, ...prev]);
            setIpSuccess('Inpatient record added successfully.');
            setShowAddIP(false);
            setIpForm({ admission_date: new Date().toISOString().split('T')[0], reason: '', ward: '', bed_number: '', attending_doctor: '', notes: '', status: 'admitted' });
        } finally {
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleSearch} className="flex gap-3">
                <div className="relative flex-1 max-w-lg">
                    ...
                    <input
                        ...
                        placeholder="Enter Patient Firestore ID (e.g. abc123...)"
                        value={patientId}
            ...
            {patient && (
                <div className="space-y-4">
                    {/* Profile Section */}
                    <SectionHeader icon={<User className="h-4 w-4 text-green-600" />} title="Patient Profile" open={openSections.profile} onClick={() => toggleSection('profile')} />
                    ...
                    <SectionHeader icon={<Bed className="h-4 w-4 text-purple-600" />} title={`Inpatient Management (${ipHistory.length} records)`} open={openSections.ip} onClick={() => toggleSection('ip')} />
                    {openSections.ip && (
                    ...
                    {/* Medical History */}
                    <SectionHeader icon={<FileText className="h-4 w-4 text-blue-600" />} title={`Medical History (${medicalRecords.length} records)`} open={openSections.history} onClick={() => toggleSection('history')} />
                    ...
                    {/* Health Metrics */}
                    <SectionHeader icon={<Activity className="h-4 w-4 text-red-500" />} title={`Health Metrics (${healthMetrics.length})`} open={openSections.metrics} onClick={() => toggleSection('metrics')} />
                    ...
                    {/* Appointments */}
                    <SectionHeader icon={<Calendar className="h-4 w-4 text-amber-500" />} title={`Appointments (${appointments.length})`} open={openSections.appointments} onClick={() => toggleSection('appointments')} />
